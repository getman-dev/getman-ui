    ...
    useEffect(() => {
        ...
        function onKeyDown(e: KeyboardEvent) {
            ...
            if (isMeta && e.key === "Enter") {
                if (playgroundSnapshot.endpoint && !playgroundSnapshot.loading) {
                    e.preventDefault();
                    executePlayground();
                }
                return;
            ...
            if (e.key === "Escape") {
                if (modalSnapshot.commandBarVisible) {
                    modalActions.setCommandBarVisible(false);
                    return;
                }
                if (modalSnapshot.shortcutsVisible) {
                    modalActions.setShortcutsVisible(false);
                    return;
                }
                if (modalSnapshot.modalVisible) {
                    modalActions.setModalVisible(false);
                    specActions.setLoadError("");
                    return;
                }
                if (authSnapshot.authModalVisible) {
                    authActions.setAuthModalVisible(false);
                    return;
                }
            ...
            if (e.key === "?") {
                e.preventDefault();
                modalActions.setShortcutsVisible(!modalSnapshot.shortcutsVisible);
            }
        }
        ...
        >
            ...
            {shortcutsVisible && (
                <div
                    ...
                    onClick={(e) => {
                        if (e.target === e.currentTarget) modalActions.setShortcutsVisible(false);
                    }}
                >
            ...
            <ErrorBoundary>
                <div id="pane-container" className="flex flex-1 min-h-0 overflow-hidden">
                    ...
                    <div id="handle-right" className={`w-1 shrink-0 cursor-col-resize group ${theme.paneHandle}`}
                         aria-hidden="true" style={{display: showPlayground ? "" : "none"}}/>
