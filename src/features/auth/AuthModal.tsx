...
function schemeTypeLabel(scheme: SecurityScheme): string {
  switch (scheme.type) {
    case "apiKey":
      return `API Key · ${scheme.in ?? "header"}`;
    case "http":
...
export default function AuthModal() {
  ...
  function authorize(schemeName: string) {
    setAuthValues({
      ...authValues,
      [schemeName]: {...(drafts[schemeName] ?? {value: "", username: "", password: ""})}
    });
  }
      ...
      <Modal visible={authModalVisible} onClose={close} title={title} footer={footer} maxWidth="max-w-lg" scrollable>
        {entries.length > 1 && (
            <div className={slot(t.tabBar)}>
              {entries.map(([name]) => (
                  <button key={name} className={activeScheme === name ? slot(t.tabActive) : slot(t.tabInactive)}
            ...
            return (
                <div key={name}>
                  ...
                  {scheme.type === "http" && scheme.scheme?.toLowerCase() === "basic" ? (
                    ...
                    return (
                        <div className="flex flex-col gap-2">
                          {authUrl && <p className={slot(t.schemeType)}>Auth URL: <code
                              className="font-mono">{authUrl}</code></p>}
                          {tokenUrl && <p className={slot(t.schemeType)}>Token URL: <code
                              className="font-mono">{tokenUrl}</code></p>}
                          <div>
                            <label className={slot(t.fieldLabel)}>Access Token</label>
                            <input type="text" className={slot(t.input)}
                      ...
                      <div>
                        {scheme.openIdConnectUrl && (
                            <p className={`${slot(t.schemeType)} mb-2`}>OpenID Connect: <code
                                className="font-mono text-[9px]">{scheme.openIdConnectUrl}</code></p>
                        )}
                        <label className={slot(t.fieldLabel)}>Access Token</label>
                      ...
                      <div>
                        <label className={slot(t.fieldLabel)}>
                          Token{scheme.bearerFormat &&
                            <span className={slot(t.fieldLabelHint)}> ({scheme.bearerFormat})</span>}
                        </label>
                        <div className={slot(t.tokenWrapper)}>
                          <span className={slot(t.tokenPrefix)}>Bearer</span>
                          <input type="text" placeholder="your-token"
