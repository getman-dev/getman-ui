...
export default function ParamField({field, value, invalid, onChange, onFileChange}: Props) {
    ...
    function renderInput() {
        ...
        if (field.type === "array" && items?.enum?.length) {
            return <MultiSelect value={value} options={items.enum} invalid={invalid}
                                onChange={(v) => onChange(name, v)}/>;
        }
        if (field.type === "array") {
            return <ArrayInput value={value} style={field.style} explode={field.explode} invalid={invalid}
                               onChange={(v) => onChange(name, v)}/>;
        }
        ...
        if (field.enum?.length) {
            return <EnumSelect value={value} options={field.enum} required={required} invalid={invalid}
                               onChange={(v) => onChange(name, v)}/>;
        }
        return (
            <ScalarInput
